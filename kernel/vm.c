...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    //清除父进程中所有PTE的PTE_W位,并设置PTE_COW位,表示所在页是一个写时复制页(多个进程引用同一个物理页)
    //如果该页面本身就是不可写(只读) ,则不会添加这个标志位
...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while(len > 0){
    ...
    if(uvmcheckcowpage(dstva))//检查每一项是否为cow页
      uvmcowcopy(dstva);
...
uvmcowcopy(uint64 va){
  ...
  if(mappages(p->pagetable,va,1,new,flags)==-1)//新的映射
    panic("uvmunmapcowpage: mappages");
  return 0;
}